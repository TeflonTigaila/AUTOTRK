...
#this part of the function is creating the file and writing on it #NEWFILE w+ represents the write+ access to the file
    file = os.path.join(f"prjfiles/{file_name}.py")
...
#remove the file from the prjfiles dir
        os.remove(f"prjfiles/{file_name}.py")
...
def run_file():
    subprocess.run(["python3",f"prjfiles/{select_run_file.get()}.py"])
...
def open_it():
    global path, i,line
    path = f"prjfiles/{select_open_file.get()}.py"
    i = 5
